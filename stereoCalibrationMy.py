'''
The code is adapted from https://github.com/TemugeB/python_stereo_camera_calibrate.git
'''
...
def calibrate_camera_for_intrinsic_parameters(images_prefix, image_type):
...
def stereo_calibrate(mtx0, dist0, mtx1, dist1, frames_prefix_c0, frames_prefix_c1):
    ...
    ret, CM1, dist0, CM2, dist1, R, T, E, F = cv.stereoCalibrate(objpoints, imgpoints_left, imgpoints_right, mtx0,
                                                                 dist0,
...
def check_calibration_static(frames_prefix_c0, camera0_data, frames_prefix_c1, camera1_data, _zshift=50.):
    cmtx0 = np.array(camera0_data[0])
    dist0 = np.array(camera0_data[1])
    R0 = np.array(camera0_data[2])
    T0 = np.array(camera0_data[3])
    ...
    for i in np.arange(len(c0_images_names)):

        # open images
        frame0 = cv.imread(c0_images_names[i])
        frame1 = cv.imread(c1_images_names[i])
...
if __name__ == '__main__':
    ...
    # camera0 intrinsics
    images_prefix = 'data/IR_2/*.png'
    cmtx0, dist0 = calibrate_camera_for_intrinsic_parameters(images_prefix, "IR")
    save_camera_intrinsics(cmtx0, dist0, 'camera0')  # this will write cmtx and dist to disk
    # camera1 intrinsics
    images_prefix = 'data/RGB_2/*.jpg'
    cmtx1, dist1 = calibrate_camera_for_intrinsic_parameters(images_prefix, "RGB")
    save_camera_intrinsics(cmtx1, dist1, 'camera1')  # this will write cmtx and dist to disk

    """Step4. Use paired calibration pattern frames to obtain camera0 to camera1 rotation and translation"""
    frames_prefix_c0 = 'data/IR_2/*.png'
    frames_prefix_c1 = 'data/RGB_2/*.jpg'
    ...
    # check_calibration_static(frames_prefix_c0, camera0_data, frames_prefix_c1, camera1_data, _zshift=60.)
